    ...
    def avg_throughput(self):
        if (self.chosen_network == 1):
            ...
            for i in range(len(self.snr_list)):
                ans += self.chi_factor*self.vho*self.shannon_capacity()[i]*min(self.proportion_of_time, 1 - (self.hho/self.cell_dwell_times[i]))
            ...
            ans = 0
            for i in range(len(self.snr_list)):
                ans += self.chi_factor*self.vho*self.shannon_capacity()[i]*self.proportion_of_time
